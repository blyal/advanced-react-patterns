...
- Compound Components share **implicit state**
  - Meaning that the children don't directly receive props in the JSX, but
    expect it anyway
  - And the state/prop handling is done inside the parent component
- use React.Children.map instead of array.prototype.map when mapping React
  children
  - Also maps if only one child rather than an array
  - More optimised (performance)
  - Maps multiple types of children
  - graceful handling of nulls and undefineds
- To add props to an element, can use React.cloneElement(element, {object:
  "ofNewProps"})
- The above two methods are useful in the Compound Components pattern
- Can perform child validation in the Compound Components parent as well (see
  extra credit + video)
- If want to pass DOM elements as children in the compound components pattern,
  should exclude them from the cloneElement (see extra credit)
- < div /> etc = DOM components; < Toggle /> etc = composite/custom components
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Patterns%20%F0%9F%A4%AF&e=02%3A%20Compound%20Components&em=alexblyth%40pm.me).
