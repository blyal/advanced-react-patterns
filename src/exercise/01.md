...
- Really simply, this is how to create helper functions for updating state when
  you use Context and useReducer together
  - It's basically a continuation/continuation of the earlier useContext and
    useReducer exercises
- This pattern is recommended by Epic React in this situation
- Could also use context function exports in useCallbacks, but seems not to be
  recommended, at least in the case of useReducer
- The video explanation for this exercise also covers returning of promises
  (remember to return them to allow the promise chain to be continued when the
  function is used) and promises vs async/await pattern
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Patterns%20%F0%9F%A4%AF&e=01%3A%20Context%20Module%20Functions&em=alexblyth%40pm.me).
